...
let bindSnapshotSelector = undefined;
try {
...
function NumberRow(props) {
    ...
    return react.createElement('div', { style: rowStyle() }, react.createElement('div', { style: { flex: 1, minWidth: 0 } }, react.createElement('div', { style: { fontSize: '14px', lineHeight: '22px' } }, label), react.createElement('div', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-tertiary, gray)', marginTop: '2px' } }, '允许范围 ' + min + ' – ' + max + ' ' + unit)), react.createElement('input', {
        type: 'number',
        value: text,
        min,
        max,
...
function ArbBucketCard(props) {
    ...
    const setField = async (field, raw) => {
        setStatus('saving');
        try {
    ...
    return react.createElement('div', { style: { padding: '4px 2px 24px', maxWidth: '640px' } }, react.createElement('h2', { style: { fontSize: '16px', fontWeight: 500, margin: '8px 0 4px' } }, L.title), react.createElement('p', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-tertiary, gray)', lineHeight: '18px', margin: '0 0 8px' } }, L.desc), react.createElement(NumberRow, {
        label: L.bytesLabel,
        unit: L.bytesUnit,
        ...
        max: 256,
        step: 0.5,
        onCommit: (mb) => void setField('maxImageBytes', Math.max(1024, Math.round(mb * MB))),
        ...
        max: 400,
        step: 1,
        onCommit: (mp) => void setField('maxImagePixels', Math.max(100_000, Math.round(mp * MP))),
    }), react.createElement(NumberRow, {
        label: L.dimLabel,
        unit: L.dimUnit,
        ...
        max: 16384,
        step: 100,
        onCommit: (px) => void setField('maxImageDimension', Math.round(px)),
    }), react.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 0 0' } }, react.createElement('button', {
        type: 'button',
...
export function apply(ctx) {
    ...
    ctx.slots.inject('settings.section', () => ctx.slots.register({
        ...
        label: () => L.nav,
        inject: injected,
    }, ArbBucketCard), PLUGIN_ID + ': settings section entry');
}

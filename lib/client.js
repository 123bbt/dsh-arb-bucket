window.__ModuleLoader__.load({
	id: "@dsh-external/dsh-arb-bucket",
	factory: (require) => {
		var module = { exports: {} };
		...
		let bindSnapshotSelector = void 0;
		try {
		...
		function NumberRow(props) {
			...
			return react.createElement("div", { style: rowStyle() }, react.createElement("div", { style: {
				flex: 1,
				minWidth: 0
				...
				fontSize: "12px",
				color: "var(--dsw-alias-label-tertiary, gray)",
				marginTop: "2px"
			} }, "允许范围 " + min + " – " + max + " " + unit)), react.createElement("input", {
				type: "number",
				value: text,
				min,
				max,
		...
		function ArbBucketCard(props) {
			...
			return react.createElement("div", { style: {
				...
				fontWeight: 500,
				margin: "8px 0 4px"
			} }, L.title), react.createElement("p", { style: {
				fontSize: "12px",
				color: "var(--dsw-alias-label-tertiary, gray)",
				lineHeight: "18px",
				margin: "0 0 8px"
			} }, L.desc), react.createElement(NumberRow, {
				label: L.bytesLabel,
				unit: L.bytesUnit,
				...
				max: 400,
				step: 1,
				onCommit: (mp) => void setField("maxImagePixels", Math.max(1e5, Math.round(mp * MP)))
			}), react.createElement(NumberRow, {
				label: L.dimLabel,
				unit: L.dimUnit,
			...
			}, L.reset), statusText ? react.createElement("span", { style: {
				fontSize: "12px",
				color: statusColor
			} }, statusText) : null));
		}

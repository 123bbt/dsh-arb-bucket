...
function findSaveImagesPrototype(store) {
    let proto = Object.getPrototypeOf(store);
    while (proto && Object.getPrototypeOf(proto) !== null) {
        if (proto.constructor && proto.constructor.name === 'AttachmentStore')
            return proto;
...
function relaxStoreImageLimits(store) {
    try {
        const current = (store.imageLimits ?? {});
...
async function pushLimits(ctx, limits, scopeTag) {
    ...
    ctx.logger.info('[dsh-arb-bucket] upload-allow limits ensured (' + scopeTag + '): ' +
...
export function apply(ctx, config) {
    ...
    const push = (tag) => pushLimits(ctx, UPLOAD_ALLOW, tag)
        .catch((e) => {
